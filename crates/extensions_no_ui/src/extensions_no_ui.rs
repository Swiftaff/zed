...
//{ExtensionManifest, ExtensionOperation};
/*
use fuzzy::{match_strings, StringMatchCandidate};
*/
use gpui::{AppContext, EventEmitter, FocusableView, Render, View, ViewContext, WeakView};
use ui::prelude::*;
/*
actions, uniform_list, Action, , ClipboardItem, , Flatten, ,
    InteractiveElement, KeyContext, ParentElement, , Styled, Task, TextStyle,
    UniformListScrollHandle,  VisualContext, , WindowContext,
};
...
//actions!(zed, [InstallDevExtension2]);
use extensions_ui::InstallDevExtension;

pub fn init(cx: &mut AppContext) {
    dbg!("init");
    cx.observe_new_views(move |workspace: &mut Workspace, cx| {
        dbg!("init observer_new_views");
        workspace.register_action(move |workspace, _: &zed_actions::ExtensionsNoUI, cx| {
            dbg!("register_action");
            let existing = workspace
                .active_pane()
                .read(cx)
                .items()
                .find_map(|item| item.downcast::<ExtensionsPageNoUI>());
            if let Some(existing) = existing {
                dbg!("1");
                workspace.activate_item(&existing, true, true, cx);
            } else {
                dbg!("2");
                let extensions_page = ExtensionsPageNoUI::new(workspace, cx);
        ...
        /*
                    .register_action(move |workspace, _: &InstallDevExtension, cx| {
                        dbg!("init observer_new_views.register_action");
        ...
        */
        /*cx.subscribe(workspace.project(), |_, _, event, cx| {
            if let extension_host::Event::DevExtensionInstallSuccess = event {
                dbg!("TESTY");
            }
        })
        .detach();
            ...
            let subscriptions = [
                ...
                cx.subscribe(&store, move |this, _, event, cx| {
                    ...
                    match event {
                        ...
                        extension_host::Event::DevExtensionInstallFailed(e) => {
                            dbg!("fail event received");
                            //this.on_dev_extension_install(workspace_handle.clone(), cx, &e.clone(), true)},
                        }
...
impl Render for ExtensionsPageNoUI {
        ...
        v_flex()
            ...
            .child(
                Button::new("install-dev-extension", "Install Dev Extension 1")
                    ...
                    .on_click(|_event, cx| cx.dispatch_action(Box::new(InstallDevExtension))),
            )
    }
}

impl EventEmitter<ItemEvent> for ExtensionsPageNoUI {}

impl FocusableView for ExtensionsPageNoUI {
    fn focus_handle(&self, cx: &AppContext) -> gpui::FocusHandle {
        self.query_editor.read(cx).focus_handle(cx)
    }
}

impl Item for ExtensionsPageNoUI {
